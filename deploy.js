...
(async function () {
  ...
  let prog = JSON.stringify(program);
  fs.writeFileSync("program.json", prog);

  const storage = Keypair.generate();
let stor = JSON.stringify(storage);
  fs.writeFileSync("program.json", stor);
...
//   console.log("program : ", program.secretKey);
//   console.log("storage : ", storage.secretKey);
//   console.log("storage : ", storage.toBuffer());
  ...
  await contract.deploy(
    "ERC20",
    ["NasriToken", "NSR", "1000000000000000000"],
    program,
    storage,
    4096 * 8
  );
  ...
  let name = await contract.functions.myname();
  console.log(name);
  ...
  // console.log(recipient.secretKey())
  await contract.transfer(
    publicKeyToHex(recipient.publicKey),
    "1000000000000000000"
  );
  console.log("data public : ", recipient.publicKey.toBase58());
  process.exit(0);
})();
